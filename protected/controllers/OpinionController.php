...
class OpinionController extends Controller {
            ...
            if ($model -> validate() && $aspect -> validate()) {
                ...
                try {
                    ...
                    $this -> redirect(array(
                        'view',
                        'id' => $model -> id_opinion,
                        'section' => $this -> _section -> id_section
                    ));
                } catch(Exception $e) {
                    $trans -> rollback();
                }
    ...
    public function actionJudgeupdate($id) {
        $opinion = $this -> loadModel($id);

        $this -> _article = $opinion -> article;
        $aspect = $opinion -> aspects[0];
        ...
        if (isset($_POST['Opinion']) && isset($_POST['OpinionAspect'])) {
            ...
            $opinion -> attributes = $_POST['Opinion'];
            $aspect -> attributes = $_POST['OpinionAspect'];

            if ($opinion -> validate() && $aspect -> validate()) {
                ...
                try {
                    ...
                    $message -> id_sender = Yii::app() -> user -> id;
                    $message -> subject = "Vélemény: " . $this -> _article -> title . " ";
                    $message -> body = "A Ön által felöltött cikk elbírálsában változások történtek. Az elbírálás jelenlegi állapota: " . ($opinion -> status == Opinion::STATUS_ACCEPT ? " elfogadva" : "viszautasítva") . "!";
    ...
    public function actionIndex() {
        ...
        $opinions = Opinion::model() -> with(array(
            ...
            'article.sectionArticles' => array(
                'joinType' => 'INNER JOIN',
                'on' => 'sectionArticles.id_section = ' . $this -> _section -> id_section
            ),
        )) -> findAll($criteria);
